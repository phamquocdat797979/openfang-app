...
async function callGeminiAPI(systemPrompt, userMessage, model) {
  const apiKey = OPENFANG_CONFIG.GEMINI_API_KEY;
  const apiModel = model || OPENFANG_CONFIG.GEMINI_MODEL;
  const url = `${OPENFANG_CONFIG.GEMINI_API_URL}/${apiModel}:generateContent?key=${apiKey}`;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  addLog('success', 'Gemini', `Gemini API đã được tích hợp (model: ${OPENFANG_CONFIG.GEMINI_MODEL})`);
  navigateTo('chat');

  // Show Gemini badge in header
  showGeminiBadge();
});
...
function createSkill() {
  ...
  addLog('success', 'Skills', `Skill mới được tạo: "${name}"`);
  document.getElementById('statSkills').textContent = state.skills.length;
}
...
function createAgent() {
  ...
  if (selectedSkillIds.length === 0) {
    ...
    showToast('Bắt buộc phải chọn ít nhất 1 skill!', 'error');
    return;
  }
  ...
  showToast(`Agent "${name}" đã được tạo thành công!`, 'success');
  addLog('success', 'Agents', `Agent mới: "${name}" (${model}) với ${selectedSkillIds.length} skills`);
}
  ...
  list.innerHTML = state.agents.map(agent => `
    <div class="agent-item" onclick="openChatWithAgent('${agent.id}')">
      ...
      <div class="agent-badges">
        <span class="badge badge-${agent.status}">${agent.status === 'running' ? 'RUNNING' : agent.status === 'crashed' ? 'CRASHED' : 'IDLE'}</span>
      </div>
...
function openChatWithAgent(agentId) {
  ...
  document.getElementById('chatAgentSkills').textContent = `Skills: ${skillNames.join(' · ')}`;
  ...
  addToSharedMemory(agentId, agent.name, 'chat_opened',
    `Người dùng bắt đầu chat với agent "${agent.name}"`);
  ...
  if (agent.messages.length === 0) {
    ...
    container.innerHTML = `
      <div class="chat-empty">
        ...
        <p style="font-size:12px;opacity:0.7">Skills: ${skillNames.join(' · ')}</p>
        <div style="margin-top:8px;padding:10px 16px;background:rgba(79,142,247,0.08);border:1px solid rgba(79,142,247,0.2);border-radius:8px;font-size:11px;color:#4F8EF7;max-width:360px;text-align:center">
          ✨ Powered by Gemini ${OPENFANG_CONFIG.GEMINI_MODEL}
        </div>
      </div>
    `;
  } else {
...
async function sendMessage() {
  ...
  try {
    ...
    const errMsg = { role: 'assistant', content: `❌ Lỗi kết nối Gemini API: ${err.message}`, timestamp: new Date() };
...
async function processAgentResponse(userInput) {
  ...
  // Call Gemini
  const response = await callGeminiAPI(systemPrompt, userInput, agent.model.startsWith('gemini') ? agent.model : OPENFANG_CONFIG.GEMINI_MODEL);
  ...
  // Determine log type
  const isRefusal = response.includes('⛔') || response.includes('chỉ có thể') || response.includes('only');
  const memType = isMemoryQuery ? 'memory_queried' : isRefusal ? 'request_refused' : 'skill_used';
  const logLevel = isRefusal ? 'warn' : 'success';

  addToSharedMemory(agent.id, agent.name, memType,
    detectedSkillName
      ? `Agent "${agent.name}" dùng skill "${detectedSkillName}" trả lời: "${userInput.substring(0, 80)}"`
      : isMemoryQuery
        ? `Agent "${agent.name}" truy vấn shared memory`
        : isRefusal
          ? `Agent "${agent.name}" từ chối: "${userInput.substring(0, 80)}"`
          : `Agent "${agent.name}" phản hồi: "${userInput.substring(0, 80)}"`
  );
  addLog(logLevel, 'Gemini', `[${agent.name}] ${detectedSkillName ? `Skill: ${detectedSkillName}` : isRefusal ? 'Refused out-of-scope' : 'Responded'}`);
...
function openAgentDetail(agentId) {
  ...
  document.getElementById('chatWithAgentBtn').onclick = () => {
    closeModal('agentDetailModal');
  ...
  const catColor = catColors[cat] || '#8b91a8';

  document.getElementById('agentDetailBody').innerHTML = `
    <div class="agent-detail-grid">
      <div class="detail-field">
        <div class="detail-field-label">TÊN AGENT</div>
        <div class="detail-field-value">${agent.name}</div>
      </div>
      <div class="detail-field">
        <div class="detail-field-label">MÔ HÌNH LLM</div>
        <div class="detail-field-value" style="font-family:var(--font-mono);font-size:12px">${agent.model}</div>
      </div>
      <div class="detail-field">
        <div class="detail-field-label">DANH MỤC</div>
        <div class="detail-field-value" style="color:${catColor}">${agent.category}</div>
      </div>
      <div class="detail-field">
        <div class="detail-field-label">TRẠNG THÁI</div>
        <div class="detail-field-value" style="color:var(--green)">● ${agent.status.toUpperCase()}</div>
      </div>
      <div class="detail-field" style="grid-column:1/-1">
        <div class="detail-field-label">MÔ TẢ</div>
        <div class="detail-field-value" style="font-weight:400;color:var(--text-secondary)">${agent.desc}</div>
      </div>
    </div>
    <div class="detail-skills-section">
      ...
      <div class="detail-skills-list">
        ${skillNames.map(n => `<span class="detail-skill-tag">${n}</span>`).join('')}
      </div>
    </div>
    ...
    <div class="memory-section" style="margin-top:12px">
      ...
      <div style="display:flex;gap:10px;margin-top:8px">
        <div class="detail-field" style="flex:1">
          <div class="detail-field-label">TIN NHẮN</div>
          <div class="detail-field-value">${agent.messages.length}</div>
        </div>
        <div class="detail-field" style="flex:1">
          <div class="detail-field-label">NGÀY TẠO</div>
          <div class="detail-field-value" style="font-size:11px">${agent.createdAt.toLocaleString('vi-VN')}</div>
        </div>
      </div>
    </div>
  `;
...
function deleteCurrentAgent() {
  ...
  if (confirm(`Bạn có chắc muốn xóa agent "${agent.name}"?`)) {
    ...
    addToSharedMemory('system', 'System', 'agent_deleted',
      `Agent "${agent.name}" đã bị xóa khỏi hệ thống`);
    renderAgentsList();
    updateStatusBar();
    updateStats();
    closeModal('agentDetailModal');
    showToast(`Agent "${agent.name}" đã bị xóa.`, 'info');
    if (state.currentChatAgentId === agentId) {
      state.currentChatAgentId = null;
      currentAgent = null;
    }
  }
}
...
function updateOverviewPage() {
  ...
  if (state.agents.length === 0) {
    overviewList.innerHTML = '<div class="placeholder-content" style="height:100px"><p>Chưa có agent nào được tạo</p></div>';
    return;
  }
  overviewList.innerHTML = state.agents.map(agent => `
    <div class="overview-agent-item">
      <div class="agent-avatar" style="width:32px;height:32px;font-size:13px;border-radius:8px">
        ${agent.name.charAt(0).toUpperCase()}
      </div>
      <div style="flex:1">
        <div style="font-weight:600">${agent.name}</div>
        <div class="overview-agent-skills">${getSkillNames(agent.skillIds).join(' · ')}</div>
      </div>
...
function renderSessionsPage() {
  ...
  if (state.agents.length === 0) {
    list.innerHTML = '<div class="placeholder-content"><p>Chưa có phiên chat nào</p></div>';
    return;
  }
  list.innerHTML = state.agents.map(agent => `
    <div class="session-item" onclick="openChatWithAgent('${agent.id}')">
      ...
      <div class="session-info">
        ...
        <div class="session-meta">${agent.model} · ${getSkillNames(agent.skillIds).join(', ')}</div>
      </div>
...
function addLog(level, source, msg) {
  state.logs.push({ level, source, msg, time: new Date() });
}

function renderLogsPage() {
  const container = document.getElementById('logsContainer');
  if (state.logs.length === 0) {
    container.innerHTML = '<div style="color:var(--text-muted);padding:8px">Chưa có logs.</div>';
    return;
  }
  container.innerHTML = [...state.logs].reverse().map(log => {
    ...
    return `<div class="log-entry">
      ...
      <span style="color:var(--accent);min-width:70px">[${log.source}]</span>
